...
CONTAINS
  ...
  SUBROUTINE GetPar( FileRoot )
    ...
    REAL               :: zMin, zMax
    REAL      (KIND=8) :: kMin, kMax, RMax
    CHARACTER (LEN=80) :: FileRoot

    ! Call READIN to get most of the environmental data

    Title = 'SPARC-   '
    CALL ReadEnvironment( FileRoot, Title, freq, MaxMedium, TopOpt, N, BotOpt, ENVFile, PRTFile )
    ...
    IF ( ( HSTop%BC /= 'V' .AND. HSTop%BC /= 'R' ) .OR. &
         ( HSBot%BC /= 'V' .AND. HSBot%BC /= 'R' ) ) THEN
           CALL ERROUT( PRTFile, 'F', 'GETPAR', 'SPARC only allows Vacuum or Rigid boundary conditions'  )
    END IF

    ! Following is irrelevant since Acousto-Elastic BCs are not allowed
    IF ( HSTop%BC == 'A' ) THEN
       HSTop%cP  = CRCI( 1D20, HSTop%alphaR, HSTop%alphaI, freq, freq, SSP%AttenUnit, HSTop%beta, HSTop%fT )
       HSTop%cS  = CRCI( 1D20, HSTop%betaR,  HSTop%betaI,  freq, freq, SSP%AttenUnit, HSTop%beta, HSTop%fT )
    END IF

    IF ( HSBot%BC == 'A' ) THEN
       HSBot%cP  = CRCI( 1D20, HSBot%alphaR, HSBot%alphaI, freq, freq, SSP%AttenUnit, HSBot%beta, HSBot%fT )
       HSBot%cS  = CRCI( 1D20, HSBot%betaR,  HSBot%betaI,  freq, freq, SSP%AttenUnit, HSBot%beta, HSBot%fT )
    END IF
    ...
    IF ( Pos%Rr( Pos%NRr ) > 1000.0 * Rmax ) THEN
       ...
       CALL ERROUT( PRTFile, 'F', 'GETPAR', 'Largest receiver range exceeds RMax'  )
    END IF
    ...
    IF ( ANY( SSP%sigma( 1 : SSP%NMedia ) /= 0.0 ) ) CALL ERROUT( PRTFile, 'F', 'GetPar', 'Rough interfaces not allowed'  )
