PROGRAM FIELD
  ...
  CASE DEFAULT
     CALL ERROUT( PRTFile, 'F', 'FIELD', 'Unknown option for source beam pattern' )
  END SELECT
  ...
  CASE DEFAULT
     CALL ERROUT( PRTFile, 'F', 'FIELD', 'Unknown option for coherent vs. incoherent mode addition' )
  END SELECT
  ...
  IF ( NRro /= Pos%Nrz ) THEN
     ...
     CALL ERROUT( PRTFile, 'F', 'FIELD', 'Number of range offsets, NRro, not equal to number of depths, NRz' )
  ENDIF
  ...
  IF ( IAllocStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'FIELD', ' Insufficient memory to allocate P( NRz, NRr )' )
