PROGRAM BOUNCE
  ...
  CALL ReadEnvironment( FileRoot, TITLE, freq, MaxMedium, TopOpt, NG, BotOpt, ENVFile, PRTFile )

  READ(  ENVFile, *    ) clow, chigh        ! Spectral limits
  WRITE( PRTFile, "( /, ' cLow = ', G12.5, ' m/s      cHigh = ', G12.5, ' m/s' )" ) cLow, cHigh

  IF ( clow == 0.0 ) CALL ERROUT( PRTFile, 'F', 'BOUNCE', 'clow must be positive' )
  ...
  ! depth of 1D20 is a large number to ensure bio loss is not included
  IF ( HSTop%BC == 'A' ) THEN
     HSTop%cP  = CRCI( 1D20, HSTop%alphaR, HSTop%alphaI, freq, freq0, SSP%AttenUnit, HSTop%beta, HSTop%fT )
     HSTop%cS  = CRCI( 1D20, HSTop%betaR,  HSTop%betaI,  freq, freq0, SSP%AttenUnit, HSTop%beta, HSTop%fT )
  END IF

  IF ( HSBot%BC == 'A' ) THEN
     HSBot%cP  = CRCI( 1D20, HSBot%alphaR, HSBot%alphaI, freq, freq0, SSP%AttenUnit, HSBot%beta, HSBot%fT )
     HSBot%cS  = CRCI( 1D20, HSBot%betaR,  HSBot%betaI,  freq, freq0, SSP%AttenUnit, HSBot%beta, HSBot%fT )
  END IF

  WRITE( PRTFile, * ) 'Writing internal reflection coeffecient table'

  ! Compute NkTab, the number of wavenumbers for which we will evaluate the reflection loss
...
CONTAINS
  ...
  SUBROUTINE ComputeReflCoef( FileRoot, kmin, kmax )
    ...
    USE BCImpedancecMod
    REAL      (KIND=8), INTENT( IN ) :: kMin, kMax
